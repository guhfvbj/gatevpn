...
- 安装后的命令行工具改为 `en`，同时保留 `eianun` 兼容别名。
- 安装脚本已适配主流 Linux 包管理器：APT、DNF、YUM、Pacman、Zypper、APK、Portage、XBPS。
- 服务管理器支持：systemd、OpenRC、runit。
- 安装/更新时都会提示是否自定义面板端口、安全后缀、账号、密码和拉取地区。
...
注意：本项目需要可用的服务管理器来注册后台服务，目前支持 **systemd / OpenRC / runit**。极简容器或裁剪系统如果没有 TUN/OpenVPN 能力，脚本会提示缺失项。
...
3. 检测包管理器：`apt-get`、`dnf`、`yum`、`pacman`、`zypper`、`apk`、`emerge`、`xbps-install`。
...
5. 安装后再次检测必要工具：`openvpn`、`curl`、`git`、服务管理器、`ip`、`ping`、`iptables`、`pkill`、`Python`。
...
en country     # 设置节点拉取地区
en update      # 从 GitHub 更新
en uninstall   # 卸载
```

兼容旧命令：`eianun` 仍会指向 `en`。旧版 `ml` 会在安装时清理。
